...
This plugin is intended to allow exporting of supplementary communications data using a `Private` element and custom namespace.

Two `Private` elements are provided, `Transpower-GSE-Receive` and `Transpower-SMV-Receive` which "mimic" the performance of the SCL defined `GSE` and `SMV` elements.

The intention of this is to allow declarative expression of communications information necessary to build network configuration.
...
. The purpose is to provide declarative information about communications which can be extracted for allow scripting for building of network configuration.

. Two `Private` elements are provided, `Transpower-GSE-Receive` and `Transpower-SMV-Receive` which "mimic" the performance of the SCL defined `GSE` and `SMV` elements.

. For each subscribing control block for an IED, one of these elements is created, using the `etp` namespace: `pass:[https://transpower.co.nz/SCL/SCD/Communication/v1]`

. Below, an example is given:
+
[source,xml]
----
<Private type="Transpower-GSE-Receive" etp:iedName="XAT_T1_P1" etp:cbName="GoCB16" etp:ldInst="Master">
    <etp:P type="VLAN-ID" xsi:type="tP_VLAN-ID">000</etp:P>
    <etp:P type="VLAN-PRIORITY" xsi:type="tP_VLAN-PRIORITY">4</etp:P>
    <etp:P type="MAC-Address" xsi:type="tP_MAC-Address">01-0C-CD-01-00-06</etp:P>
</Private>
----

. In general, the element and attributes match those for the `GSE` and `SMV` as defined in the standard, however:

* An additional field is used to include the name of the publishing IED, `etp:iedName`.
* These elements are only added if a prior `ConnectedAP` exists.
