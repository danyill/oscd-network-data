...
. Two `Private` elements are provided, `Transpower-GSE-Subscribe` and `Transpower-SMV-Subscribe` which "mimic" the performance of the SCL defined `GSE` and `SMV` elements.
